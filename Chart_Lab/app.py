...
PAD, MARGIN = 20, 0.05         # x축 오른쪽 여백, y축 상ㆍ하 여유 %
...
def start_random_modelbook():
    ...
    path = None
    for p in (os.path.join(root, "modelbook.txt"),
    ...
    if not tickers:
        st.error("modelbook.txt 에 유효한 티커가 없습니다."); return
...
mas = [( "EMA", int(p), True) for p in ema_in.split(",") if p.strip().isdigit()] + \
      [( "SMA", int(p), True) for p in sma_in.split(",") if p.strip().isdigit()]
clr = {("EMA",10):"red", ("EMA",21):"blue",
       ("SMA",50):"orange", ("SMA",200):"black"}
pal = ["#1f77b4","#ff7f0e","#2ca02c","#d62728","#9467bd","#8c564b","#e377c2","#7f7f7f"]

# ── 데이터 준비
visible = add_mas(g.df.copy(), mas).iloc[: g.idx+1]
...
if "view_n" not in st.session_state:
    st.session_state.view_n = 120
view_n = st.columns([9,1])[1].number_input(
    "표시봉", 50, len(df_trade), st.session_state.view_n, 10, label_visibility="collapsed")
...
ymin = sub[["Low"]+[f"{k}{p}" for k,p,_ in mas]].min().min()
ymax = sub[["High"]+[f"{k}{p}" for k,p,_ in mas]].max().max()
...
# 볼륨
vol_c = ["black" if c<=o else "white"
         for o,c in zip(df_trade.Open, df_trade.Close)]
fig.add_bar(x=df_trade.i, y=df_trade.Volume,
            marker_color=vol_c, marker_line_color="black", marker_line_width=.5,
